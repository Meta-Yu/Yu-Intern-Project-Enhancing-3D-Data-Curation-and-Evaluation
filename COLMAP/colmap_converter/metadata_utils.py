...
def split_ids(ids, nth_image=8):
    ...
    if nth_image != 0:
        ...
        ids_te = []
        ids_vl = []
    ids_tr = sorted(set(ids).difference(set(ids_vl).union(ids_te)))
...
def reduce_ids(meta):
    ...
    def fn_reduce(x, min_id):
        return (x - min_id) // 50

    assert len({fn_reduce(x, min_id) for x in meta["ids_all"]}) == len(
        set(meta["ids_all"])
    )
    for k in meta:
        if "ids" in k:
...
def calc_meta(
    ...
    def fn2int(fn):
        if "frame" in fn:
            fn = fn[len("frame"):]
        return int(os.path.splitext(fn.split("/")[-1])[0])
    ...
    assert list(colmap2fn.keys()) == sorted(colmap2fn.keys())
    ...
    if to_PT3d:
        poses[:, 0:2] *= -1
        poses[:, :3, :3] = poses[:, :3, :3].transpose((0, 2, 1))
...
def save_meta(directory, meta, name="meta.json", deepcopy=False):
    def tolist(x):
        return x if type(x) is list else x.tolist()
